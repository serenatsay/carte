...
export const useCarteStore = create<CarteState>((set) => ({
  ...
  increment: (sectionId, itemId) => set((state) => {
    const existing = state.cart[itemId];
    const quantity = (existing?.quantity ?? 0) + 1;
    return {
      cart: { ...state.cart, [itemId]: { itemId, quantity, sectionId, isWildcard: existing?.isWildcard, wildcardReason: existing?.wildcardReason } },
    };
  }),
  ...
  clear: () => set({ cart: {} }),
  addWildcardSelections: (selections) => set((state) => {
    const newCart = { ...state.cart };
